...
import (
	...
	"Users/evilkidz/Project/Golang/CMS/backend/modules/content/controller"
...
type Handler struct {
	fx.In
	Controller controller.IContentController
...
func (receiver *Handler) Route(m ...echo.MiddlewareFunc) {
	echoRoute := receiver.Router.Group("/v1/content", m...)
...
func (receiver *Handler) Create(c echo.Context) error {
	var reqData = new(content.CreateRequest)
	...
	if err := _permission.Create(data.Permissions, models.FeatureContent); err != nil {
...
func (receiver *Handler) FindAll(c echo.Context) error {
	var reqData = new(content.FindAllRequest)
	...
	if err := _permission.Read(data.Permissions, models.FeatureContent); err != nil {
		receiver.Logger.Error(err)
		return utilities.Response(c, &utilities.ResponseRequest{
			StatusCode: http.StatusForbidden,
			Message:    utilities.Forbidden,
