...
import (
	...
	"gitlab.com/tiketfest/backend/modules/transactionItem/controller"
...
type Handler struct {
	fx.In
	Controller controller.ITransactionItemController
...
func (receiver *Handler) Route(m ...echo.MiddlewareFunc) {
	echoRoute := receiver.Router.Group("/v1/transaction/item", m...)
...
func (receiver *Handler) Create(c echo.Context) error {
	var reqData = new(transactionItem.CreateRequest)
