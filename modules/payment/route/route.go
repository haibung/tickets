...
import (
	...
	"gitlab.com/tiketfest/backend/modules/payment/controller"
...
func (receiver *Handler) Route(m ...echo.MiddlewareFunc) {
	echoRoute := receiver.Router.Group("/v1/payment", m...)
...
func (receiver *Handler) Create(c echo.Context) error {
	var reqData = new(payment.CreateRequest)
	...
	if err := _permission.Create(data.Permissions, models.FeaturePayment); err != nil {
