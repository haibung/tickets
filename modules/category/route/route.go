...
import (
	...
	"Users/evilkidz/Project/Golang/CMS/backend/modules/category/controller"
...
func (receiver *Handler) Route(m ...echo.MiddlewareFunc) {
	echoRoute := receiver.Router.Group("/v1/category", m...)
...
func (receiver *Handler) Create(c echo.Context) error {
	var reqData = new(category.CreateRequest)
	...
	if err := _permission.Create(data.Permissions, models.FeatureCategory); err != nil {
...
func (receiver *Handler) FindAll(c echo.Context) error {
	var reqData = new(category.FindAllRequest)
	...
	if err := _permission.Read(data.Permissions, models.FeatureCategory); err != nil {
		receiver.Logger.Error(err)
		return utilities.Response(c, &utilities.ResponseRequest{
			StatusCode: http.StatusForbidden,
			Message:    utilities.Forbidden,
